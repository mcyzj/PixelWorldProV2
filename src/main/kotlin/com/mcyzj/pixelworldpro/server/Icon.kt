package com.mcyzj.pixelworldpro.server

import org.bukkit.Bukkit

object Icon {
    fun pixelWorldPro(){
        Bukkit.getConsoleSender().sendMessage("§a______  _              _  _    _               _      _ ______              ")
        Bukkit.getConsoleSender().sendMessage("§a| ___ \\(_)            | || |  | |             | |    | || ___ \\             ")
        Bukkit.getConsoleSender().sendMessage("§a| |_/ / _ __  __  ___ | || |  | |  ___   _ __ | |  __| || |_/ / _ __   ___  ")
        Bukkit.getConsoleSender().sendMessage("§a|  __/ | |\\ \\/ / / _ \\| || |/\\| | / _ \\ | '__|| | / _` ||  __/ | '__| / _ \\ ")
        Bukkit.getConsoleSender().sendMessage("§a| |    | | >  < |  __/| |\\  /\\  /| (_) || |   | || (_| || |    | |   | (_) |")
        Bukkit.getConsoleSender().sendMessage("§a\\_|    |_|/_/\\_\\ \\___||_| \\/  \\/  \\___/ |_|   |_| \\__,_|\\_|    |_|    \\___/ ")
    }
    fun v2Alpha(){
        Bukkit.getConsoleSender().sendMessage("§e                      §c  .----------------.  .----------------.  .----------------.  .----------------.  .----------------. ")
        Bukkit.getConsoleSender().sendMessage("§e __     __   ______   §c | .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |")
        Bukkit.getConsoleSender().sendMessage("§e|  \\   |  \\ /      \\  §c | |      __      | || |   _____      | || |   ______     | || |  ____  ____  | || |      __      | |")
        Bukkit.getConsoleSender().sendMessage("§e| \$\$   | \$\$|  \$\$\$\$\$\$\\ §c | |     /  \\     | || |  |_   _|     | || |  |_   __ \\   | || | |_   ||   _| | || |     /  \\     | |")
        Bukkit.getConsoleSender().sendMessage("§e| \$\$   | \$\$ \\\$${"$"}__| \$\$ §c | |    / /\\ \\    | || |    | |       | || |    | |__) |  | || |   | |__| |   | || |    / /\\ \\    | |")
        Bukkit.getConsoleSender().sendMessage("§e \\\$\$\\ /  \$\$ /      \$\$ §c | |   / ____ \\   | || |    | |   _   | || |    |  ___/   | || |   |  __  |   | || |   / ____ \\   | |")
        Bukkit.getConsoleSender().sendMessage("§e  \\\$\$\\  \$\$ |  \$\$\$\$\$\$  §c | | _/ /    \\ \\_ | || |   _| |__/ |  | || |   _| |_      | || |  _| |  | |_  | || | _/ /    \\ \\_ | |")
        Bukkit.getConsoleSender().sendMessage("§e   \\\$\$ \$\$  | \$${"$"}_____  §c | ||____|  |____|| || |  |________|  | || |  |_____|     | || | |____||____| | || ||____|  |____|| |")
        Bukkit.getConsoleSender().sendMessage("§e    \\\$\$\$   | \$\$     \\ §c | |              | || |              | || |              | || |              | || |              | |")
        Bukkit.getConsoleSender().sendMessage("§e     \\\$     \\\$\$\$\$\$\$\$\$ §c | '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |")
        Bukkit.getConsoleSender().sendMessage("§e                      §c  '----------------'  '----------------'  '----------------'  '----------------'  '----------------' ")
    }
    fun warning(){
        Bukkit.getConsoleSender().sendMessage("§c .----------------.  .----------------.  .----------------.  .----------------.  .----------------. ")
        Bukkit.getConsoleSender().sendMessage("§c| .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |")
        Bukkit.getConsoleSender().sendMessage("§c| |      __      | || |   _____      | || |   ______     | || |  ____  ____  | || |      __      | |")
        Bukkit.getConsoleSender().sendMessage("§c| |     /  \\     | || |  |_   _|     | || |  |_   __ \\   | || | |_   ||   _| | || |     /  \\     | |")
        Bukkit.getConsoleSender().sendMessage("§c| |    / /\\ \\    | || |    | |       | || |    | |__) |  | || |   | |__| |   | || |    / /\\ \\    | |")
        Bukkit.getConsoleSender().sendMessage("§c| |   / ____ \\   | || |    | |   _   | || |    |  ___/   | || |   |  __  |   | || |   / ____ \\   | |")
        Bukkit.getConsoleSender().sendMessage("§c| | _/ /    \\ \\_ | || |   _| |__/ |  | || |   _| |_      | || |  _| |  | |_  | || | _/ /    \\ \\_ | |")
        Bukkit.getConsoleSender().sendMessage("§c| ||____|  |____|| || |  |________|  | || |  |_____|     | || | |____||____| | || ||____|  |____|| |")
        Bukkit.getConsoleSender().sendMessage("§c| |              | || |              | || |              | || |              | || |              | |")
        Bukkit.getConsoleSender().sendMessage("§c| '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |")
        Bukkit.getConsoleSender().sendMessage("§c '----------------'  '----------------'  '----------------'  '----------------'  '----------------' ")
    }
}